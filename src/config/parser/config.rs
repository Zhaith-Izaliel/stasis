...
pub async fn load_merged_config() -> Result<RuneConfig, ConfigParseError> {
    ...
    if let Some(user_path) = user_path {
        if user_path.exists() {
            ...
            event_debug_scoped!(Scope::Config, "Loaded config from: {}", user_path.display());
            return Ok(config);
        }
    }
    ...
    if system_path.exists() {
        ...
        event_debug_scoped!(Scope::Config, "Loaded config from: {}", system_path.display()); 
        return Ok(config);
    }
    ...
    if share_path.exists() {
        ...
        event_debug_scoped!(Scope::Config, "Loaded config from: {}", share_path.display());
        return Ok(config);
    }
    
    event_debug_scoped!(Scope::Config, "Using internal default configuration");
    Ok(config)
}
...
pub async fn load_config() -> Result<StasisConfig, ConfigParseError> {
    ...
    parse_base_stasis_config(&config).await
}
...
pub async fn load_combined_config() -> Result<CombinedConfig, ConfigParseError> {
    let config = load_merged_config().await?;
    let base = parse_base_stasis_config(&config).await?;
    ...
    if !profiles.is_empty() {
        ...
        event_info_scoped!(Scope::Config, "Profiles loaded: {}", profile_names.join(", "));
    }
